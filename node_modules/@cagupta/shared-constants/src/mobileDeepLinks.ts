import { normalizePortalSlug, type PortalKey } from './portals.js';
...
export const MOBILE_UNIVERSAL_LINK_HOSTS = ['app.carahulgupta.in'] as const;

export type MobileDeepLinkLogin = { type: 'login'; portal: PortalKey | null };
...
function pathFromParsedUrl(parsed: URL): string {
  const host = parsed.hostname || '';
  const segments = parsed.pathname.split('/').filter(Boolean);
  if (MOBILE_UNIVERSAL_LINK_HOSTS.includes(host as (typeof MOBILE_UNIVERSAL_LINK_HOSTS)[number])) {
    return segments.join('/');
  }
  if (!host && segments.length === 0) return '';
  if (!host) return segments.join('/');
  if (segments.length === 0) return host;
  return [host, ...segments].join('/');
}

function portalFromSegment(segment: string): PortalKey | null {
  if (segment === 'core' || segment === 'staff') return 'staff';
  return normalizePortalSlug(segment);
}

/** Build `carahulgupta://…` URL for emails, push payloads, or QR codes. */
export function buildMobileDeepLink(path: string, query?: Record<string, string>): string {
...
export function buildMobileLoginLink(portal: PortalKey): string {
 ...
 */
export function parseMobileDeepLink(url: string | null | undefined): MobileDeepLink | null {
  if (!url) return null;
  try {
    ...
    const isUniversalLink =
      scheme === 'https' &&
      MOBILE_UNIVERSAL_LINK_HOSTS.includes(parsed.hostname as (typeof MOBILE_UNIVERSAL_LINK_HOSTS)[number]);
...
export function resolveInitialPortalFromUrl(url: string | null | undefined): PortalKey | null {
  const link = parseMobileDeepLink(url);
  if (!link) return null;
  if (link.type === 'login') return link.portal;
  return link.portal;
}

/** Map auth role to portal key for deep-link permission checks. */
export function portalKeyForRole(role: string | null | undefined): PortalKey {
