...
export const VALID_PORTALS: readonly PortalKey[] = [
  'staff',
  'associate',
  'client',
  'partner',
...
export const PORTAL_META: Record<PortalKey, PortalMetaEntry> = {
  staff: {
    label: 'Core',
    accent: '#2563eb',
    tint: '#eff6ff',
...
export function isPortalKey(value: string | null | undefined): value is PortalKey {
  return VALID_PORTALS.includes(value as PortalKey);
}

/** Normalize URL ?portal= slug (handles legacy affiliate → associate). */
export function normalizePortalSlug(raw: string | null | undefined): PortalKey | null {
  if (!raw) return null;
...
export function portalMismatchMessage(portal: PortalKey, user: PortalUser | null | undefined): string {
...
export function mobileNavigatorForRole(role: string | null | undefined): MobilePortalNavigator {
  if (role === 'associate') return 'Associate';
  if (role === 'partner') return 'Partner';
  if (role === 'client') return 'Client';
  return 'Core';
