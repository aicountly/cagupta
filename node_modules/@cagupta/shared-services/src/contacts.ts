...
function coerceIdArray(val: unknown): number[] {
  if (val == null) return [];
...
function coerceNameArray(val: unknown): string[] {
  if (val == null) return [];
  if (Array.isArray(val)) {
...
function lifecycleStatusFromApi(c: Record<string, unknown>): string {
  const cs = String(c.contact_status ?? c.status ?? '').trim().toLowerCase();
  if (cs === 'active' || cs === 'inactive' || cs === 'prospect') return cs;
  return c.is_active === false ? 'inactive' : 'active';
}

export function normalizeContact(c: Record<string, unknown>): ContactRow {
  ...
  const id = c.id as number | string;
...
export function createContactsService(api: ApiClient) {
  return {
    async getContactsWithMeta(params: {
      ...
      const data = await api.get<Record<string, unknown>[]>('/admin/contacts', {
        page: params.page,
