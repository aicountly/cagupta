...
const nav = [
  { to: '/associate', label: 'Dashboard', end: true },
  { to: '/associate/services', label: 'My services' },
  { to: '/associate/commissions', label: 'Commissions' },
  { to: '/associate/payouts', label: 'Payouts' },
  { to: '/associate/rewards', label: 'Rewards' },
  { to: '/associate/chat', label: 'Team chat' },
  { to: '/associate/bank', label: 'Bank / KYC' },
  { to: '/associate/sub-associates', label: 'Invite associate' },
  { to: '/associate/profile', label: 'Profile' },
];

export default function AssociateLayout({ title, children }) {
  const navigate = useNavigate();
  const { logout, session } = useAuth();
  const name = session?.user?.name || 'Associate';
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--portal-bg)' }}>
      ...
      >
        <div style={{ padding: 16, borderBottom: '1px solid #f1f5f9' }}>
          ...
          <div style={{ fontSize: 11, color: '#64748b', marginTop: 8 }}>Associate portal</div>
        </div>
