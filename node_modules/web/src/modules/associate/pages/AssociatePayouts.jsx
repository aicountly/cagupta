...
import { getAssociatePayoutRequests, postAssociatePayoutRequest, getAssociateDashboard } from '../services/associatePortalService';

export default function AssociatePayouts() {
  ...
  function refresh() {
    getAssociatePayoutRequests().then(setRows).catch(() => setRows([]));
    getAssociateDashboard().then((d) => setAvail(Number(d.available_balance || 0))).catch(() => {});
  }
  ...
  async function submit(e) {
    ...
    try {
      const res = await postAssociatePayoutRequest({ max_amount: n, fast_track: fast });
  ...
  return (
    <AssociateLayout title="Payout requests">
