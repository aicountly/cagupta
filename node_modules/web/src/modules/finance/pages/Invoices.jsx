...
import {
  getRecoveryLogs, createRecoveryLog,
    ...
    <div style={overlayStyle}>
      <div style={{ ...modalStyle, maxWidth: 700 }}>
        ...
        <div style={{ padding:'20px 24px', display:'flex', flexDirection:'column', gap:14 }}>
          ...
          {form.entityId ? (
            <label style={labelStyle}>
              Link to service engagement (optional — for associate commissions)
              <select
                style={inputStyle}
              ...
              >
                ...
                {engagementOptions.map((eng) => (
                  <option key={eng.id} value={String(eng.id)}>
                    #{eng.id} · {eng.type || 'Service'}{eng.referringAssociateUserId ? ' · associate' : ''}
                  </option>
                ))}
              </select>
            </label>
...
function EditLedgerTxnModal({ txnId, onClose, onSaved }) {
  ...
  const withinUserRevWindow = row && row.createdAt
    ...
    && new Date(row.createdAt).getTime() >= nowMs - 30 * 86400000;
...
export default function Invoices({ ledgerOnly = false }) {
  ...
  const buildTxnListParams = useCallback((targetTab, page) => {
