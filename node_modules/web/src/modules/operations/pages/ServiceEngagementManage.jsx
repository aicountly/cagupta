...
export default function ServiceEngagementManage() {
  ...
  const [, setMasterOrgId] = useState(null);
  ...
  const [referringAssociateUserId, setReferringAssociateUserId] = useState('');
  ...
  useEffect(() => {
    ...
    getEngagement(id)
      .then(eng => {
        ...
        setReferringAssociateUserId(eng.referringAssociateUserId != null ? String(eng.referringAssociateUserId) : '');
  ...
  const associateReadonlyLabel = useMemo(() => {
    if (!referringAssociateUserId) return 'None';
    const a = approvedAssociates.find((x) => String(x.id) === String(referringAssociateUserId));
    return a ? `${a.name} (${a.email})` : `Linked user #${referringAssociateUserId}`;
  }, [referringAssociateUserId, approvedAssociates]);
      ...
      <div style={tabPanel}>
        ...
        <section style={sectionCard}>
          ...
          <p style={{ fontSize: 12, color: '#64748b', margin: '0 0 12px', lineHeight: 1.45 }}>
            Referring associate and commission mode are fixed for this engagement. Referral start date is maintained on the client record. To change the client-facing flag, or to match removing tasks on the Tasks tab, request a superadmin OTP and enter it before saving.
          </p>
          <label style={fieldLabel}>
            Referring associate
            <div style={{ ...inputStyle, background: '#f8fafc', color: '#334155', cursor: 'default' }}>{associateReadonlyLabel}</div>
          </label>
          <label style={{ ...fieldLabel, marginTop: 14 }}>
            Commission mode
