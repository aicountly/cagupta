...
final class AffiliateRedemptionRequestModel
{
    private PDO $db;
    ...
    public function createPending(int $userId, string $catalogKey, int $points): int
    {
        $stmt = $this->db->prepare(
            'INSERT INTO affiliate_redemption_requests (affiliate_user_id, catalog_key, points, status)
    ...
    public function listForAffiliate(int $userId): array
    {
        $stmt = $this->db->prepare(
            'SELECT * FROM affiliate_redemption_requests WHERE affiliate_user_id = :u ORDER BY created_at DESC'
        );
    ...
    public function listAdmin(string $status, int $page, int $per): array
    {
        ...
        $off = ($page - 1) * $per;
        $c = $this->db->prepare("SELECT COUNT(*) FROM affiliate_redemption_requests WHERE {$where}");
        ...
        $stmt = $this->db->prepare(
            "SELECT r.*, u.name, u.email
             FROM affiliate_redemption_requests r
    ...
    {
        $stmt = $this->db->prepare('SELECT * FROM affiliate_redemption_requests WHERE id = :id');
        $stmt->execute([':id' => $id]);
    ...
    public function setStatus(int $id, string $status, ?int $fulfilledBy, ?string $notes): bool
    {
        ...
        $stmt = $this->db->prepare(
            'UPDATE affiliate_redemption_requests SET
                status = :st,
                fulfilled_by = :fb,
