...
final class AffiliateRewardLedgerModel
{
    private PDO $db;
    ...
    public function insertRow(array $r): void
    {
        $stmt = $this->db->prepare(
            'INSERT INTO affiliate_reward_ledger (affiliate_user_id, delta_points, kind, ref_type, ref_id, label)
             VALUES (:u, :dp, :k, :rt, :rid, :lb)'
        );
        $stmt->execute([
            ':u'   => (int)$r['affiliate_user_id'],
    ...
    {
        $stmt = $this->db->prepare(
            'SELECT COALESCE(SUM(delta_points), 0) FROM affiliate_reward_ledger WHERE affiliate_user_id = :u'
        );
    ...
    {
        $this->db->prepare(
            'DELETE FROM affiliate_reward_ledger
             WHERE ref_type = \'downline_commission\' AND ref_id = :iid'
        )->execute([':iid' => $invoiceTxnId]);
    }

    /** @return array<int, array<string, mixed>> */
    public function listForAffiliate(int $userId, int $limit = 80): array
    {
        $stmt = $this->db->prepare(
            'SELECT * FROM affiliate_reward_ledger
