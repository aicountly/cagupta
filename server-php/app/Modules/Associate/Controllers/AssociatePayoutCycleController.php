...
final class AssociatePayoutCycleController extends BaseController
{
    ...
    /** GET /api/admin/associate-payout-cycles?year=2026 */
    public function index(): never
    {
        ...
        $rows = (new AssociatePayoutCycleModel())->listForYearMerged($year);
        $this->success($rows);
    }

    /** POST /api/admin/associate-payout-cycles/ensure { period_end: "YYYY-MM-DD" } */
    public function ensure(): never
    {
        ...
        try {
            $row = (new AssociatePayoutCycleModel())->ensureOpenForPeriodEnd($pe);
    ...
    /** GET /api/admin/associate-payout-cycles/:id */
    public function show(int $id): never
    {
        $this->assertPerm();
        $m   = new AssociatePayoutCycleModel();
        $row = $m->find($id);
        if ($row === null) {
        ...
        $pending = (new AssociatePayoutCycleAmendmentModel())->findPendingForCycle($id);
        $this->success([
            'cycle'             => $row,
            'lines'             => $lines,
    ...
    /** GET /api/admin/associate-payout-cycles/:id/preview */
    public function preview(int $id): never
    {
        $this->assertPerm();
        try {
            $preview = (new AssociatePayoutCycleModel())->buildPreview($id);
        } catch (\InvalidArgumentException $e) {
    ...
    /** POST /api/admin/associate-payout-cycles/:id/finalise */
    public function finalise(int $id): never
    {
        ...
        $this->audit('associate_payout_cycle.finalised', $id);
        $this->success($cycles->find($id), 'Cycle finalised');
    }

    /** POST /api/admin/associate-payout-cycles/:id/disburse */
    public function disburse(int $id): never
    {
        ...
        $this->audit('associate_payout_cycle.disbursed', $id);
        $this->success($cycles->find($id), 'Cycle marked disbursed');
    }

    /**
     * POST /api/admin/associate-payout-cycles/:id/amendments
    ...
    public function submitAmendment(int $id): never
    {
        ...
        $this->audit('associate_payout_cycle_amendment.submitted', $aid);
        ...
        if ($uids !== []) {
            try {
                (new UserNotificationModel())->createForUsers(
                    ...
                    'Associate payout cycle amendment pending',
                    'Cycle #' . $id . ' (' . $cycle['period_start'] . ' → ' . $cycle['period_end'] . ') requires Super Admin approval.',
                    'associate_payout_cycle_amendment',
                    $aid
                );
            } catch (\Throwable $e) {
                error_log('[AssociatePayoutCycle] notify superadmins: ' . $e->getMessage());
            }
        }
    ...
    private function audit(string $action, int $entityId): void
    {
        ...
        try {
            ...
            error_log('[AssociatePayoutCycle] audit: ' . $e->getMessage());
        }
    }
}
