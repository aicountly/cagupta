...
final class AssociatePayoutCycleAmendmentModel
{
    private PDO $db;
    ...
    public function insertPending(int $cycleId, int $requestedByUserId, array $adjustments, ?string $requestReason = null): int
    {
        $stmt = $this->db->prepare(
            'INSERT INTO associate_payout_cycle_amendments (
                associate_payout_cycle_id, status, adjustments_json, requested_by_user_id, request_reason
    ...
    {
        $stmt = $this->db->prepare('SELECT * FROM associate_payout_cycle_amendments WHERE id = :id');
        $stmt->execute([':id' => $id]);
        $row = $stmt->fetch();
    ...
    {
        $stmt = $this->db->prepare(
            "SELECT * FROM associate_payout_cycle_amendments
             WHERE associate_payout_cycle_id = :cid AND status = 'pending'
    ...
    public function listPendingWithCycle(): array
    {
        $stmt = $this->db->query(
            "SELECT a.*,
             ...
             FROM associate_payout_cycle_amendments a
             JOIN associate_payout_cycles c ON c.id = a.associate_payout_cycle_id
    ...
    {
        $stmt = $this->db->prepare(
            "UPDATE associate_payout_cycle_amendments
