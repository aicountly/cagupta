...
final class AssociatePayoutCycleModel
{
    private PDO $db;
    ...
    {
        $stmt = $this->db->prepare('SELECT * FROM associate_payout_cycles WHERE id = :id');
        $stmt->execute([':id' => $id]);
        $row = $stmt->fetch();
    ...
    public function findByPeriod(string $periodStart, string $periodEnd): ?array
    {
        $stmt = $this->db->prepare(
            'SELECT * FROM associate_payout_cycles WHERE period_start = :ps AND period_end = :pe LIMIT 1'
        );
    ...
    public function insertOpen(array $seg): int
    {
        ...
        $stmt = $this->db->prepare(
            'INSERT INTO associate_payout_cycles (
    ...
    public function listForYearMerged(int $year): array
    {
        ...
        $stmt     = $this->db->prepare(
            'SELECT * FROM associate_payout_cycles
    ...
    {
        $stmt = $this->db->prepare(
            'SELECT l.*, u.name AS associate_name, u.email AS associate_email
             FROM associate_payout_cycle_lines l
             JOIN users u ON u.id = l.associate_user_id
             WHERE l.associate_payout_cycle_id = :cid
             ORDER BY l.id ASC'
        );
    ...
    /**
     * @return array{accruals: array<int, array<string, mixed>>, by_associate: array<int, array{user_id: int, name: string, total: float}>}
     */
    public function buildPreview(int $cycleId): array
    {
        ...
        $accruals = (new CommissionAccrualModel())->listEligibleForAssociatePayoutCycle(
            (string)$cycle['period_start'],
            (string)$cycle['period_end']
        );
        $byAff = [];
        foreach ($accruals as $r) {
            $uid = (int)$r['associate_user_id'];
            if (!isset($byAff[$uid])) {
                $byAff[$uid] = [
                    'user_id' => $uid,
                    'name'    => '',
        ...
        return ['accruals' => $accruals, 'by_associate' => array_values($byAff)];
    }
    ...
    public function finaliseWithAmountMap(int $cycleId, int $actorId, array $amountFinalByAccrualId): void
    {
        ...
        try {
            $stmt = $this->db->prepare(
                'SELECT * FROM associate_payout_cycles WHERE id = :id FOR UPDATE'
            );
            ...
            $eligible = $accrualModel->listEligibleForAssociatePayoutCycle(
                (string)$cycle['period_start'],
                (string)$cycle['period_end']
            );
            ...
            $lineStmt  = $this->db->prepare(
                'INSERT INTO associate_payout_cycle_lines (
                    associate_payout_cycle_id, commission_accrual_id, associate_user_id,
            ...
            $updAcc = $this->db->prepare(
                "UPDATE commission_accruals
                 SET amount = :amt,
                     status = 'in_payout',
                     associate_payout_cycle_id = :cid,
                     metadata = COALESCE(metadata, '{}'::jsonb) || CAST(:meta AS jsonb)
                 WHERE id = :id AND status = 'accrued' AND associate_payout_cycle_id IS NULL"
            );
            ...
            $updCyc = $this->db->prepare(
                "UPDATE associate_payout_cycles
    ...
    public function disburse(int $cycleId, int $actorId): void
    {
        ...
        try {
            ...
            $pay = $this->db->prepare(
                "UPDATE commission_accruals
                 SET status = 'paid'
                 WHERE associate_payout_cycle_id = :cid AND status = 'in_payout'"
            );
    ...
    public function runSlaNotifications(UserNotificationModel $notifications): int
    {
        ...
        foreach ($rows as $c) {
            ...
            $title = '';
            $body  = sprintf(
                'Associate payout cycle #%d (%s → %s). Disburse by %s.',
                $id,
                $c['period_start'],
                $c['period_end'],
                $due
            ...
            if ($today > $due && empty($c['sla_overdue_notified_at'])) {
                $title = 'Overdue: associate payout disbursement';
                $body .= ' This cycle is past the 3-day disbursement window.';
                $this->db->prepare(
                    'UPDATE associate_payout_cycles SET sla_overdue_notified_at = NOW(), updated_at = NOW() WHERE id = :id'
                )->execute([':id' => $id]);
            } elseif ($today >= $d24 && $today <= $due && empty($c['sla_reminder_24h_at'])) {
                $title = 'Reminder: associate payout due soon';
                $this->db->prepare(
                    'UPDATE associate_payout_cycles SET sla_reminder_24h_at = NOW(), updated_at = NOW() WHERE id = :id'
                )->execute([':id' => $id]);
            } elseif ($today >= $d48 && $today < $d24 && empty($c['sla_reminder_48h_at'])) {
                $title = 'Reminder: associate payout disbursement window';
                $this->db->prepare(
                    'UPDATE associate_payout_cycles SET sla_reminder_48h_at = NOW(), updated_at = NOW() WHERE id = :id'
                )->execute([':id' => $id]);
            }
            ...
            if ($title !== '') {
                $notifications->createForUsers(
                    $uids,
                    'associate_payout_cycle_sla',
                    $title,
                    $body,
                    'associate_payout_cycle',
                    $id
                );
                ++$sent;
            }
