...
final class AssociateUplineTrackerModel
{
    private PDO $db;
    ...
    public function get(int $parentUserId, int $childUserId): ?array
    {
        $stmt = $this->db->prepare(
            'SELECT * FROM associate_upline_sub_tracker
    ...
    {
        $stmt = $this->db->prepare(
            'INSERT INTO associate_upline_sub_tracker (parent_user_id, child_user_id, cumulative_child_commission, blocks_paid)
             VALUES (:p, :c, :cum, :bp)
