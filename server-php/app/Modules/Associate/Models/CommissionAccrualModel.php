...
final class CommissionAccrualModel
{
     ...
     */
    public function sumChildInvoiceCommissions(int $childAssociateUserId): float
    {
        $stmt = $this->db->prepare(
        ...
        $stmt->execute([':uid' => $childAssociateUserId]);
    ...
    public function insert(array $row): int
    {
        $stmt = $this->db->prepare(
            'INSERT INTO commission_accruals (
                associate_user_id, invoice_txn_id, service_id, accrual_type, accrual_date,
                commission_mode, tier_used, net_fee_base, rate_percent, amount, currency,
                status, child_associate_user_id, metadata
            ) VALUES (
                :associate_user_id, :invoice_txn_id, :service_id, :accrual_type, :accrual_date,
                :commission_mode, :tier_used, :net_fee_base, :rate_percent, :amount, :currency,
                :status, :child_associate_user_id, CAST(:metadata AS jsonb)
            ) RETURNING id'
        );
        $stmt->execute([
            ...
            ':child_associate_user_id'=> $row['child_associate_user_id'] ?? null,
     ...
     */
    public function paginateForAssociate(
        int $associateUserId,
        int $page = 1,
        int $perPage = 30,
     ...
     */
    public function availableBalance(int $associateUserId): float
    {
        $stmt = $this->db->prepare(
            "SELECT COALESCE(SUM(ca.amount), 0)
             FROM commission_accruals ca
             WHERE ca.associate_user_id = :uid
     ...
     * @return array<int, array<string, mixed>>
     */
    public function listAvailableForPayoutFifo(int $associateUserId): array
    {
        $stmt = $this->db->prepare(
    ...
    public function sumUplineBonusesForPair(int $parentUserId, int $childUserId): float
    {
        $stmt = $this->db->prepare(
            "SELECT COALESCE(SUM(amount), 0) FROM commission_accruals
             WHERE associate_user_id = :p
               AND child_associate_user_id = :c
    ...
    /**
     * Accruals available to sweep into an associate payout cycle for [period_start, period_end].
     *
     * @return array<int, array<string, mixed>>
     */
    public function listEligibleForAssociatePayoutCycle(string $periodStart, string $periodEnd): array
    {
        $stmt = $this->db->prepare(
            "SELECT ca.* FROM commission_accruals ca
             WHERE ca.status = 'accrued'
               AND ca.accrual_date >= :ps AND ca.accrual_date <= :pe
               AND ca.associate_payout_cycle_id IS NULL
               AND NOT EXISTS (
                   SELECT 1 FROM payout_request_lines prl
                   JOIN payout_requests pr ON pr.id = prl.payout_request_id
                   WHERE prl.commission_accrual_id = ca.id
