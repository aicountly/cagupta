...
final class PayoutRequestModel
{
    ...
    public function listForAssociate(int $associateUserId): array
    {
        $stmt = $this->db->prepare(
            'SELECT * FROM payout_requests WHERE associate_user_id = :uid ORDER BY created_at DESC'
        );
    ...
    public function listAll(string $status = '', int $page = 1, int $perPage = 30): array
    {
        ...
        $stmt = $this->db->prepare(
            "SELECT pr.*, u.name AS associate_name, u.email AS associate_email
             FROM payout_requests pr
             JOIN users u ON u.id = pr.associate_user_id
             WHERE {$whereClause}
     ...
     */
    public function createWithLines(int $associateUserId, float $totalAmount, bool $fastTrack, array $lines): int
    {
        $this->db->beginTransaction();
        try {
            $stmt = $this->db->prepare(
                'INSERT INTO payout_requests (associate_user_id, requested_amount, status, fast_track)
