...
class UserModel
{
    ...
    public function create(array $data): int
    {
        ...
        $stmt = $this->db->prepare(
            'INSERT INTO users (name, email, password_hash, role_id, is_active, is_email_verified,
                                login_provider, sso_provider_id, avatar_url, created_by, shift_target_minutes, shift_target_disabled)
        ...
        $stmt->execute([
            ...
            ':shift_target_disabled' => ($data['shift_target_disabled'] ?? false) ? 'true' : 'false',
        ]);
    ...
    public function update(int $id, array $data): bool
    {
        ...
        $allowed = ['name', 'email', 'role_id', 'is_active', 'avatar_url', 'last_login_at', 'sso_provider_id', 'planned_billable_rate_per_hour', 'shift_target_minutes', 'shift_target_disabled'];
        foreach ($allowed as $field) {
            if (array_key_exists($field, $data)) {
                $setClauses[]       = "{$field} = :{$field}";
                if ($field === 'is_active' || $field === 'shift_target_disabled') {
