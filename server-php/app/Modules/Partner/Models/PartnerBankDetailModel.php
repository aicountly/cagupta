...
final class PartnerBankDetailModel
{
    private PDO $db;
    ...
    public function listByUserId(int $userId): array
    {
        $stmt = $this->db->prepare(
            'SELECT id, user_id, account_holder_name, bank_name, account_number_last4, ifsc,
             ...
             FROM partner_bank_details WHERE user_id = :uid ORDER BY is_primary DESC, id ASC'
        );
    ...
    {
        $stmt = $this->db->prepare('SELECT * FROM partner_bank_details WHERE id = :id');
        $stmt->execute([':id' => $id]);
        $row = $stmt->fetch();
    ...
    {
        $this->db->prepare(
            'UPDATE partner_bank_details SET is_primary = FALSE WHERE user_id = :uid AND id != :kid'
    ...
    {
        $stmt = $this->db->prepare(
            'UPDATE partner_bank_details SET verification_status = :st, verified_by = :vb,
    ...
    {
        $stmt = $this->db->prepare('DELETE FROM partner_bank_details WHERE id = :id AND user_id = :uid');
