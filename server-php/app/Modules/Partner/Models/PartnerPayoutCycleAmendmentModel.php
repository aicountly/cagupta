...
final class PartnerPayoutCycleAmendmentModel
{
    private PDO $db;
    ...
    {
        $stmt = $this->db->prepare('SELECT * FROM partner_payout_cycle_amendments WHERE id = :id');
        $stmt->execute([':id' => $id]);
        $row = $stmt->fetch();
    ...
    {
        $stmt = $this->db->prepare(
            "SELECT * FROM partner_payout_cycle_amendments
             WHERE partner_payout_cycle_id = :cid AND status = 'pending'
    ...
    public function listPendingWithCycle(): array
    {
        $stmt = $this->db->query(
            "SELECT a.*,
             ...
             FROM partner_payout_cycle_amendments a
             JOIN partner_payout_cycles c ON c.id = a.partner_payout_cycle_id
    ...
    {
        $stmt = $this->db->prepare(
            "UPDATE partner_payout_cycle_amendments
