...
import { getAffiliateRewards, postAffiliateRedeem } from '../services/affiliatePortalService';
...
export default function AffiliateRewards() {
  ...
  const load = () => getAffiliateRewards().then(setData).catch((e) => setErr(e.message || 'Failed'));

  useEffect(() => { load(); }, []);

  const submit = () => {
    setErr('');
    postAffiliateRedeem({ catalog_key: catalogKey, points: parseInt(pts, 10) || 0 })
      .then(() => { setPts(''); load(); })
      .catch((e) => setErr(e.message));
  };

  return (
    <AffiliateLayout title="Rewards">
