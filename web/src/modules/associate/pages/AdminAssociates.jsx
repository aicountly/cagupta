...
import {
  getAssociatesAdmin, approveAssociate, suspendAssociate,
  getCommissionDefaults, putCommissionDefaults,
  getPayoutRequestsAdmin, patchPayoutRequest,
} from '../services/associateAdminService';

const TABS = [
  { key: 'pending', label: 'Pending', icon: Users },
  { key: 'all', label: 'All Associates', icon: Handshake },
...
export default function AdminAssociates() {
  ...
  const loadList = useCallback(() => {
    ...
    getAssociatesAdmin({ status: st, perPage: 100 })
      .then(setList)
    ...
    <div style={pageWrap}>
      ...
      <div style={headerCard}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div>
            <h1 style={{ margin: 0, fontSize: 20, fontWeight: 700, color: '#0B1F3B' }}>Associates</h1>
            <p style={{ margin: '3px 0 0', fontSize: 13, color: '#64748b' }}>
              Manage associate members, outreach pipeline, commissions, and payouts
            </p>
          </div>
        </div>
        <button type="button" onClick={() => navigate('/finance/associate-payout-cycles')} style={btnSecondary}>
          View Payout Cycles →
        </button>
      </div>
          ...
          <>
            <div style={{ padding: '14px 20px', borderBottom: '1px solid #F1F5F9', display: 'flex', gap: 10, alignItems: 'center' }}>
              <div style={{ position: 'relative', flex: 1, maxWidth: 300 }}>
                ...
                <input type="text" placeholder="Search associates..." value={search} onChange={(e) => setSearch(e.target.value)} style={{ ...inputStyle, paddingLeft: 32 }} />
              </div>
              ...
              <table style={tableStyle}>
                ...
                <tbody>
                    ...
                    <tr key={row.user_id} style={{ borderBottom: '1px solid #F8FAFC' }}>
                      ...
                      <td style={tdStyle}>
                        <div style={{ display: 'flex', gap: 6 }}>
                          {row.status === 'pending' && (
                            <button type="button" style={btnApprove} onClick={() => approveAssociate(row.user_id).then(loadList)}>Approve</button>
                          )}
                          {row.status === 'approved' && (
                            <button type="button" style={btnDanger} onClick={() => suspendAssociate(row.user_id).then(loadList)}>Suspend</button>
                          )}
                        </div>
                      </td>
                    </tr>
                  ...
                  {filteredList.length === 0 && (
                    <tr><td colSpan={6} style={{ ...tdStyle, textAlign: 'center', color: '#94a3b8' }}>No associates found.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
              ...
              <tbody>
                {payouts.map((p) => (
                  <tr key={p.id} style={{ borderBottom: '1px solid #F8FAFC' }}>
                    ...
                    <td style={tdStyle}>{p.associate_name} <span style={{ color: '#94a3b8', fontSize: 11 }}>{p.associate_email}</span></td>
...
function DefaultsForm({ defaults, onSaved }) {
  ...
  const fields = [
    ...
    ['direct_associate_pct', 'Direct Mode — Associate %'],
    ...
    <div style={{ maxWidth: 480 }}>
      ...
      <button
        ...
        onClick={() => putCommissionDefaults({
          ...
          direct_associate_pct: parseFloat(f.direct_associate_pct),
