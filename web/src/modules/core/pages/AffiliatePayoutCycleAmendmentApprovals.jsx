...
export default function AffiliatePayoutCycleAmendmentApprovals() {
  const { user } = useAuth();
  const allowed =
  ...
  async function onApprove(id) {
    ...
    setErr('');
    try {
      await approveAffiliatePayoutCycleAmendment(id);
      await load();
    } catch (e) {
  ...
  async function onReject(id) {
    const reason = window.prompt('Rejection reason (required):', rejectReason);
    ...
    setErr('');
    try {
      await rejectAffiliatePayoutCycleAmendment(id, String(reason).trim());
      setRejectReason('');
      await load();
    } catch (e) {
  ...
  if (!allowed) {
    return (
      <div style={{ padding: 24 }}>
        <h1 style={{ fontSize: 18 }}>Affiliate payout amendments</h1>
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900 }}>
      <h1 style={{ fontSize: 20, marginBottom: 8 }}>Affiliate payout cycle amendments</h1>
      <p style={{ fontSize: 13, color: '#64748b', marginBottom: 20 }}>
        Approving applies Accounts&apos; proposed commission amounts and <strong>finalises</strong> the cycle in one step (PR4).
      </p>
        ...
        return (
          ...
          >
            <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 6 }}>
              Amendment #{r.id} · Cycle #{r.affiliate_payout_cycle_id}
            </div>
            ...
            <div style={{ marginTop: 10, fontSize: 12, fontFamily: 'monospace', background: '#f8fafc', padding: 10, borderRadius: 8, overflowX: 'auto' }}>
              {adj.map((a) => (
                <div key={a.commission_accrual_id}>
                  accrual {a.commission_accrual_id} → ₹{Number(a.amount_final).toFixed(2)}
                  {a.note ? ` (${a.note})` : ''}
                </div>
              ))}
            </div>
