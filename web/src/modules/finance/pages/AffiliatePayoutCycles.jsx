...
function AmendmentForm({ cycleId, onSuccess }) {
  ...
  async function submit(e) {
    ...
    setBusy(true); setErr('');
    try {
      await submitAffiliatePayoutCycleAmendment(cycleId, adjustments, reason);
...
function CycleDetailPanel({ cycle, onAction }) {
  ...
  async function executeDestructivePrompt() {
    ...
    setDestructiveErr('');
    try {
      if (destructivePrompt === 'finalise') await finaliseAffiliatePayoutCycle(cycle.id);
      else await disburseAffiliatePayoutCycle(cycle.id);
