...
export default function AffiliatePayoutCycles() {
  ...
  const allowed = hasPermission('affiliates.manage');
  ...
  const [adjJson, setAdjJson] = useState('[\n  { "commission_accrual_id": 0, "amount_final": 0, "note": "" }\n]');
  ...
  async function openSegment(row) {
    ...
    try {
      ...
      if (!cycleId) {
        const ensured = await ensureAffiliatePayoutCycle(row.period_end);
        cycleId = ensured?.id;
      }
      if (!cycleId) {
      ...
      const [djson, pjson] = await Promise.all([
        getAffiliatePayoutCycle(cycleId),
        previewAffiliatePayoutCycle(cycleId),
      ]);
      setDetail(djson);
      setPreview(pjson);
    } catch (e) {
  ...
  async function refreshPanel() {
    ...
    try {
      const [djson, pjson] = await Promise.all([
        getAffiliatePayoutCycle(id),
        previewAffiliatePayoutCycle(id),
      ]);
      setDetail(djson);
      setPreview(pjson);
      await loadYear();
  ...
  async function onFinalise() {
    if (!cycle?.id) return;
    if (!window.confirm('Finalise this cycle at system-calculated amounts and reserve all eligible accruals?')) return;
    setBusy('fin');
    setPanelErr('');
    try {
      await finaliseAffiliatePayoutCycle(cycle.id);
      await refreshPanel();
    } catch (e) {
  ...
  async function onDisburse() {
    ...
    setPanelErr('');
    try {
      await disburseAffiliatePayoutCycle(cycle.id);
      await refreshPanel();
    } catch (e) {
  ...
  async function onSubmitAmendment() {
    ...
    setPanelErr('');
    try {
      await submitAffiliatePayoutCycleAmendment(cycle.id, adjustments);
      await refreshPanel();
    } catch (e) {
  ...
  if (!allowed) {
    return (
      <div style={{ padding: 24 }}>
        <h1 style={{ fontSize: 18 }}>Affiliate payout cycles</h1>
        <p style={{ color: '#64748b' }}>Requires affiliates.manage (Accounts or Admin).</p>
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: 24, maxWidth: 1200 }}>
      <h1 style={{ fontSize: 20, marginBottom: 8 }}>Affiliate payout cycles</h1>
      <p style={{ fontSize: 13, color: '#64748b', marginBottom: 16, maxWidth: 720 }}>
        Cycles end on the <strong>8th</strong>, <strong>15th</strong>, <strong>23rd</strong>, and <strong>month-end</strong>.
        Accounts finalises accrued commissions into a cycle, then marks disbursement after payment.
        Amount changes vs system calculations require a Super Admin amendment approval first.
        Disburse within <strong>3 calendar days</strong> after period end (SLA reminders go to Accounts + Super Admin).
      </p>
        ...
        <div style={{ marginTop: 24, ...card }}>
          ...
          {detail?.cycle && !panelLoading && (
            <>
              ...
              {st === 'open' && !detail.pending_amendment && preview?.accruals?.length > 0 && (
                <div style={{ marginBottom: 16 }}>
                  ...
                  <p style={{ fontSize: 12, color: '#64748b', marginBottom: 8 }}>
                    Only include rows where <code>amount_final</code> differs from the system amount. Use accrual ids from the preview table.
                  </p>
                  <textarea
                    value={adjJson}
              ...
              {preview?.by_affiliate?.length > 0 && (
                <div style={{ marginBottom: 12 }}>
                  <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 8 }}>Totals by affiliate (eligible accrued)</div>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                    <thead>
                      <tr style={{ color: '#64748b', textAlign: 'left' }}>
                        <th style={{ padding: 6, borderBottom: '1px solid #e2e8f0' }}>Affiliate</th>
                        <th style={{ padding: 6, borderBottom: '1px solid #e2e8f0' }}>Accrued total</th>
                      </tr>
                    </thead>
                    <tbody>
                      {preview.by_affiliate.map((x) => (
                        <tr key={x.user_id}>
              ...
              {preview?.accruals?.length > 0 && (
                <div style={{ maxHeight: 280, overflow: 'auto' }}>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
                    <thead>
                      <tr style={{ color: '#64748b', textAlign: 'left' }}>
                        {['ID', 'Affiliate', 'Date', 'Amount', 'Type'].map((h) => (
                    ...
                    <tbody>
                      {preview.accruals.map((a) => (
                        <tr key={a.id}>
                          ...
                          <td style={{ padding: 4, borderBottom: '1px solid #f8fafc' }}>{a.affiliate_user_id}</td>
                          ...
                          <td style={{ padding: 4, borderBottom: '1px solid #f8fafc' }}>{(a.accrual_type || '').replace(/_/g, ' ')}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
                    <thead>
                      <tr style={{ color: '#64748b' }}>
                        {['Accrual', 'Affiliate', 'System', 'Final'].map((h) => (
                    ...
                    <tbody>
                      {detail.lines.map((ln) => (
                        <tr key={ln.id}>
                          <td style={{ padding: 4 }}>{ln.commission_accrual_id}</td>
                          <td style={{ padding: 4 }}>{ln.affiliate_name || ln.affiliate_user_id}</td>
