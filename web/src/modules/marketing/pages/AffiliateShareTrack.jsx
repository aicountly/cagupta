...
function buildUtmUrl(affiliateId, affiliateName, medium, landingPath) {
  if (!affiliateId) return '';
  const slug = String(affiliateName || affiliateId)
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '_')
    .slice(0, 40);
...
export default function AffiliateShareTrack() {
    ...
    (async () => {
      try {
        const [aff, src] = await Promise.all([
          getAffiliatesAdmin({ status: 'approved', perPage: 200 }),
  ...
  const selectedAffiliate = affiliates.find((a) => String(a.id) === String(selected));
  const utmUrl = selected
    ? buildUtmUrl(selected, selectedAffiliate?.name, medium, landing)
    : '';
    ...
    <div style={s.page}>
      ...
      <div style={s.header}>
        ...
        <div>
          ...
          <p style={s.pageSub}>
            Generate UTM-tagged links for each affiliate and monitor referral traffic.
          </p>
        </div>
      </div>
      ...
      {referralChannel && (
        <div style={s.summaryCard}>
          ...
          <div style={s.summaryItem}>
            ...
            <div>
              <div style={{ ...s.summaryNum, color: '#2563eb' }}>{affiliates.length}</div>
              <div style={s.summaryLabel}>Active affiliates</div>
            </div>
          </div>
          <div style={s.summaryItem}>
      ...
      <div style={s.twoCol}>
        ...
        <div style={s.card}>
          ...
          <div style={s.fieldGroup}>
            <label style={s.label}>Affiliate</label>
            <select
              style={s.select}
              value={selected}
          ...
          {utmUrl && (
            <div style={s.utmBreakdown}>
              ...
              {[
                ['utm_source',   'affiliate'],
                ['utm_medium',   medium],
                ['utm_campaign', `aff_${selected}`],
                ['utm_content',  selectedAffiliate?.name?.toLowerCase().replace(/[^a-z0-9]+/g, '_').slice(0, 40) ?? ''],
              ].map(([k, v]) => (
        ...
        <div style={s.card}>
          ...
          <ol style={s.howToList}>
            {[
              'Select an affiliate from the dropdown.',
              'Choose the medium (how the affiliate will share — social, WhatsApp, email, etc.).',
              'Pick a landing page (usually Home or Services).',
              'Copy the generated URL and share it with the affiliate via WhatsApp or email.',
              'When visitors arrive via this link, GA4 automatically attributes the session to that affiliate\'s campaign.',
          ...
          <div style={s.tipBox}>
            ...
            <p style={s.tipBody}>
              ...
              <em>Referral</em> channel to see combined affiliate traffic. For per-affiliate
              attribution, use GA4's Acquisition report and filter by{' '}
              <code>utm_campaign</code> matching <code>aff_&#123;id&#125;</code>.
            </p>
          </div>
        </div>
      </div>
      ...
      {affiliates.length > 0 && (
        <div style={s.card}>
          <h2 style={s.cardTitle}>All Active Affiliates — Quick Links</h2>
          <table style={s.table}>
            <thead>
              <tr>
