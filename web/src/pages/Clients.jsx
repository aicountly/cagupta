          ...
          <tbody>
            {filtered.map(c => (
              <tr key={c.id} style={trStyle} onClick={()=>setSelected(c)}>
                ...
                <td style={{ ...tdStyle, fontWeight:600, color:'#1d4ed8', cursor:'pointer' }}>{c.displayName}</td>
...
const thStyle = { textAlign:'left', padding:'10px 12px', color:'#64748b', fontWeight:600, fontSize:12, borderBottom:'2px solid #f1f5f9', whiteSpace:'nowrap', background:'#f8fafc' };
const tdStyle = { padding:'10px 12px', color:'#334155', verticalAlign:'middle', whiteSpace:'nowrap' };
const trStyle = { borderBottom:'1px solid #f8fafc', cursor:'pointer', transition:'background 0.1s' };
const inputStyle = { flex:1, padding:'8px 12px', border:'1px solid #e2e8f0', borderRadius:8, fontSize:13, outline:'none' };
...
const btnOutline = { padding:'8px 12px', background:'#fff', color:'#2563eb', border:'1px solid #2563eb', borderRadius:8, cursor:'pointer', fontSize:13, fontWeight:600 };
