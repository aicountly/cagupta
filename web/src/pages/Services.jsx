  ...
  return (
    <div style={kpiCard}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 10 }}>
        {(() => {
          const trendColor = trendNeutral ? '#94a3b8' : trendPositive ? '#55B848' : '#ef4444';
        ...
        <span style={{ fontSize: 11, fontWeight: 600, color: trendNeutral ? '#94a3b8' : trendPositive ? '#55B848' : '#ef4444' }}>
            ...
            <table style={tableStyle}>
              ...
              <tbody>
                {filteredServices.map((s, idx) => {
                  ...
                  if (isActive) rowBg = '#EBF3FB';
                  else if (isHover) rowBg = '#F0F6FA';
                  return (
                    <tr
                      key={s.id}
              ...
              <div key={t.id} style={taskRow}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, flex: 1, minWidth: 0 }}>
                    <div style={{ ...taskDot, background: t.status === 'done' ? '#55B848' : t.status === 'in_progress' ? '#145886' : '#e2e8f0', marginTop: 4, flexShrink: 0 }} />
...
function ActionBtn({ icon: Icon, title }) {
  ...
  return (
    <button
      ...
      style={{ ...actionBtn, background: hover ? '#EBF3FB' : '#F6F7FB', borderColor: hover ? '#B0D4EC' : '#E6E8F0', color: hover ? '#145886' : '#64748b' }}
    >
      <Icon size={13} />
    </button>
  );
...
const btnPrimary = { display: 'flex', alignItems: 'center', gap: 6, padding: '8px 16px', background: '#145886', color: '#fff', border: 'none', borderRadius: 8, cursor: 'pointer', fontSize: 13, fontWeight: 600, boxShadow: '0 2px 8px rgba(20,88,134,0.30)' };
const btnSecondary = { display: 'flex', alignItems: 'center', gap: 4, padding: '4px 10px', background: '#F6F7FB', color: '#145886', border: '1px solid #B0D4EC', borderRadius: 6, cursor: 'pointer', fontSize: 12, fontWeight: 600 };
...
const serviceTag = {
  background: '#EBF3FB', color: '#145886',
...
const progressFill = { height: '100%', background: 'linear-gradient(90deg, #145886 0%, #1d7fbf 100%)', borderRadius: 99, transition: 'width 0.4s ease' };
