...
export default function ClientCompletedServices() {
  const [rows, setRows] = useState([]);
  const [err, setErr] = useState('');

  useEffect(() => {
    getClientServices({ group: 'completed', perPage: 100 }).then((r) => setRows(r.rows)).catch((e) => setErr(e.message || 'Failed'));
  }, []);

  return (
    <ClientLayout title="Completed services">
          ...
          <tbody>
            {rows.map((s) => (
              <tr key={s.id} style={{ borderTop: '1px solid #f1f5f9' }}>
                ...
                <td style={{ padding: 10 }}>{s.updated_at || '-'}</td>
                <td style={{ padding: 10 }}><Link to={`/client/services/${s.id}`}>View details</Link></td>
              </tr>
            ))}
          </tbody>
        </table>
        {rows.length === 0 && !err && <div style={{ padding: 24, color: '#64748b' }}>No completed services.</div>}
      </div>
    </ClientLayout>
  );
}
